...
def get_root_mean_square(trajectory):
    positions = trajectory['positions']

    origin = np.array(positions[0])
    rms_list = []
    for position in positions[1:-1]:
        rms = distance.euclidean(origin, np.array(position))**2
        rms_list.append(rms)

    return np.sqrt(np.average(np.array(rms_list)))


def initial_final_distance(trajectory):
    origin = np.array(trajectory['positions'][0])
    destiny = np.array(trajectory['positions'][-1])

    return distance.euclidean(origin, destiny)


def duration(trajectory):
    return trajectory['time_advance'][-1] - trajectory['time_advance'][0]


def stadistics(sample_set):
